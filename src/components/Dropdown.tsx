...
interface DropdownProps {
  color?: 'green' | 'trans-green' | 'light' | 'peach';
  options: string[];
  placeholder?: string;
...
const DropdownForNavBar: React.FC<
  DropdownProps & { isFullWidth: boolean; devSize: 'sm' | 'lg' }
> = ({
  color = 'green', // Assign a default value here
  options,
  placeholder,
  ...
  const colorEffect = {
    peach: {
      parent: 'bg-[#FFE1B9] text-green-primary',
      icon: 'fill-green-primary',
      'child-container': 'bg-[#FFE1B9] z-10',
  ...
  return (
    <div className='cursor-pointer relative' ref={dropDownRef}>
      {/* Main div unaffected by open state and placeholder */}
      <div
        className={`block w-${isFullWidth ? 'full' : '[256px]'} p-[1.5px] ${
          open ? 'rounded-t-md' : 'rounded-md'
        } ${open ? colorEffect[color].parent : 'bg-transparen text-white'}`}
        onClick={() => setOpen(!open)}
      >
        <div
          className={`rounded flex justify-between items-center w-${
            devSize == 'lg' ? '[9.2em]' : 'full'
          } py-3 px-4 lg:py-4 lg:px-5 bg-[#FFE1B9]`}
        >
          <p className='text-sm lg:text-md  font-inter capitaliz font-semibold text-green-primary'>
            EVENT
          </p>
          <ArrowDropdownIcon
            size={10}
            className={`${
              open
                ? colorEffect[color].icon
                : 'fill-green-primary animate-bounce'
            } w-4 h-4 ${
              open ? 'rotate-180' : 'rotate-0'
            } transition-all duration-300`}
          />
      ...
      <div
        className={`${
          open
            ...
            : 'absolute -translate-y-[60px] pointer-events-none opacity-0'
        } transition-all duration-300 overflow-y-scroll custom-scrollbar lg:top-[70px] mb-2 left-0 w-full rounded-b-md ${
          colorEffect[color]['child-container']
        }`}
      >
        {/* Mapping options */}
