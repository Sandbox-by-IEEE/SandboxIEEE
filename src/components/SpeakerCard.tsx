...
import { type Speaker } from '@/types/grand-seminar';

const SpeakersCard: React.FC<Speaker> = ({
  name,
  instagramUsername,
  imageSpeaker,
  explanationSpeaker,
  ...
  return (
    <div
      className={`SpeakersCard w-[400px] h-[800px] rounded-3xl block overflow-hidden relative`}
    >
      <div className='BaganFoto w-full h-[45%] overflow-hidden justify-center relative'>
        ...
        <Image
          src={imageSpeaker.url}
          alt={imageSpeaker.title}
          width={imageSpeaker.width}
          height={imageSpeaker.height}
      ...
      <div className='Description w-full h-[calc(45%)] bg-[#071D10] p-6 pt-4'>
        <div className='outerBadge w-full h-fit rounded-[30px] bg-gradient-light-brown drop-shadow-[0_0px_10px_rgba(219,184,139,0.7)] flex'>
          <div className='innerBadge w-[calc(100%-8px)] h-fit mt-[4px] mb-[4px] m-auto rounded-[28px] bg-gradient-light-cards pl-5 pr-5 block items-center overflow-hidden min-h-fit pt-3 pb-3'>
            <h4 className='w-full font-poppins text-center text-[#FFE1B9] drop-shadow-[0_0px_10px_rgba(200,170,78,1)] font-semibold text-2xl'>
              {name}
            </h4>
        ...
        <div className='w-full h-max pt-5 text-justify font-poppins font-normal text-[13pt] overflow-auto text-[#FFE1B9]'>
          <StructuredText data={explanationSpeaker} />
        </div>
      </div>
