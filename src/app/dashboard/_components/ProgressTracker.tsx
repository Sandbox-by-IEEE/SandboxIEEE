...
interface ProgressTrackerProps {
  currentPhase: 'registration' | 'preliminary' | 'payment' | 'semifinal' | 'final';
...
export default function ProgressTracker({
  ...
  const getCurrentPhaseIndex = () => {
    if (currentPhase === 'registration' || currentPhase === 'preliminary') return 0;
            ...
            return (
              <div key={phase.key} className="flex items-center">
                <div className="flex flex-col items-center">
                  <div
                    className={`w-10 h-10 rounded-full flex items-center justify-center font-bold text-sm transition-all duration-300 ${step === currentIndex + 1
                ...
                {step < phases.length && (
                  <div
                    className={`w-12 md:w-20 h-1 mx-2 rounded-full transition-all ${step < currentIndex + 1
