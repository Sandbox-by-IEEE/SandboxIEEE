  ...
  try {
    ...
    await prisma.finalSubmission.update({
      ...
      data: {
        ...
        reviewNotes: feedback || 'Final submission approved',
      },
    });
        ...
        <body>
          <div class="container">
            ...
            <div class="content">
              ...
              <p class="text">Your final submission for <strong style="color: #FFCD8D;">${submission.registration.competition.name}</strong> has been accepted!</p>
              <div style="text-align: center;"><span class="status-badge">\u2705 ACCEPTED</span></div>
              <div class="info-box">
    ...
    await sendMail({
      ...
      subject: `🎉 Final Submission Accepted - ${submission.registration.competition.name}`,
      html: emailHtml,
    });
    ...
    return NextResponse.json({
      success: true,
      message: 'Final submission approved and email sent',
    });
  } catch (error) {
    console.error('Approve final submission error:', error);
    return NextResponse.json(
