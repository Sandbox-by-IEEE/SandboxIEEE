...
export default function FinanceTable({
  ...
  currentStatus,
  currentCompetition,
}: FinanceTableProps): React.JSX.Element {
  ...
  const filtered = useMemo(() => {
    ...
    return registrations.filter(
      (r) =>
        ...
        r.competition.code.toLowerCase().includes(q),
    );
  ...
  const getCompetitionBadge = (code: string) => {
    const colors: Record<string, string> = {
      BCC: 'bg-blue-100 text-blue-700',
      PTC: 'bg-purple-100 text-purple-700',
      TPC: 'bg-orange-100 text-orange-700',
    };
    return (
      <span
        className={`inline-block px-2.5 py-0.5 rounded-full text-xs font-semibold ${
          colors[code] || 'bg-gray-100 text-gray-700'
        }`}
      >
        {code}
      </span>
      ...
      <div className='bg-white rounded-xl border border-gray-200 overflow-hidden'>
        ...
        <div className='border-b border-gray-200 bg-gray-50 p-4 space-y-3'>
          ...
          <div className='relative'>
            ...
            <input
              type='text'
              placeholder='Search by team name, account name, email, or bill name...'
              value={searchQuery}
          ...
          <div className='flex flex-wrap gap-2'>
            ...
            <div className='flex gap-1 bg-white rounded-lg p-1 border border-gray-200'>
              {[
                ...
                <a
                  key={filter.value}
                  href={`/admin/finance?status=${filter.value}&competition=${currentCompetition}`}
            ...
            <div className='flex gap-1 bg-white rounded-lg p-1 border border-gray-200'>
              <a
                href={`/admin/finance?status=${currentStatus}&competition=all`}
                className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
              ...
              {competitions.map((comp) => (
                <a
                  key={comp.code}
                  href={`/admin/finance?status=${currentStatus}&competition=${comp.code}`}
                  className={`px-3 py-1.5 rounded-md text-xs font-medium transition-all ${
                    currentCompetition === comp.code
                      ? 'bg-blue-600 text-white shadow-sm'
                      : 'text-gray-600 hover:text-gray-900 hover:bg-gray-50'
                  }`}
                >
                ...
                {filtered.map((reg) => (
                  <React.Fragment key={reg.id}>
                    ...
                    >
                      ...
                      <td className='px-4 py-3'>
                        {getCompetitionBadge(reg.competition.code)}
                      </td>
                      <td className='px-4 py-3'>
                                  ...
                                  {reg.team.members.map((m) => (
                                    ...
                                    >
                                      ...
                                      <div>
                                        ...
                                        {m.institution && (
                                          <span className='text-gray-500 ml-2 inline-flex items-center gap-1'>
                                            <Building2 size={12} />
                                            {m.institution}
                                          </span>
                                        )}
