...
const COMPETITION_FIELDS: Record<
  string,
...
> = {
  ...
  TPC: {
    label: 'Technovate Paper Competition',
    fields: [
      { key: 'paperUrl', label: 'Research Paper' },
  ...
  BCC: {
    ...
    fields: [
      { key: 'businessPlanUrl', label: 'Business Plan' },
...
export default function SubmissionsTable({
  ...
  const getSubmissionLinks = (submission: any) => {
    const compCode =
      submission.competitionType ||
      submission.registration.competition.code.toUpperCase();
    const config = COMPETITION_FIELDS[compCode];

    if (!config) return null;

    return config.fields
      .filter((f) => submission[f.key])
      .map((f) => ({
        label: f.label,
        url: submission[f.key] as string,
      }));
  };
                    ...
                    <td className='px-4 py-4'>
                      <div>
                        ...
                        <p className='text-sm text-gray-500'>
                          {submission.registration.team?.members?.[0]
                            ?.fullName || submission.registration.user.name}
                        </p>
          ...
          <div className='bg-white rounded-lg p-6 max-w-md w-full mx-4'>
            ...
            <p className='text-sm text-gray-600 mb-4'>
              {feedbackModal.action === 'approve'
                ? 'Team will be notified and moved to the final phase. Add optional feedback:'
            ...
            <textarea
              ...
              placeholder={
                feedbackModal.action === 'approve'
                  ? 'Excellent work! Your semifinal submission is outstanding...'
